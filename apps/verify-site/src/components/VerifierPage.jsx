...
export function VerifierPage() {
    ...
    const handleVerify = useCallback((json) => {
        setError(null);
        setResult(null);
        try {
    ...
    const handleDrop = useCallback((e) => {
        ...
        reader.onload = () => handleVerify(reader.result);
    ...
    return (<div className="max-w-3xl mx-auto p-6" data-testid="verifier-page">
      ...
      {/* File drop zone */}
      <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center mb-4" onDrop={handleDrop} onDragOver={(e) => e.preventDefault()} data-testid="drop-zone">
        <p className="text-gray-500">Drop VPEC JSON here</p>
      </div>

      {/* Paste area */}
      <textarea className="w-full border rounded p-3 font-mono text-xs h-32 mb-4" placeholder="Or paste VPEC JSON here..." onChange={handlePaste} data-testid="paste-area"/>

      {error && (<div className="text-red-600 mb-4" data-testid="parse-error">
          {error}
        </div>)}

      {/* Landing summary */}
      {result && (<div className="mb-6" data-testid="landing-summary">
          ...
          <div className="space-y-1 text-sm">
            <div>
              Coverage:{" "}
              {String(result.coverage.policy_coverage_pct ?? "—")}%
            </div>
            <div>
              Proof Level:{" "}
      ...
      <div className="mt-8 border-t pt-4 text-xs text-gray-500" data-testid="p1-disclaimer">
...
//# sourceMappingURL=VerifierPage.js.map