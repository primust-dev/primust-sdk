export function VerificationPanel({ result, artifact }) {
    const configDriftCount = artifact?.gaps
        ?.filter((g) => g.gap_type === "policy_config_drift").length ?? 0;
    // Count witnessed records from proof distribution
    const witnessedRecords = typeof result.proof_distribution.witnessed === "number"
        ? result.proof_distribution.witnessed
        : 0;
    return (<div className="space-y-4" data-testid="verification-panel">
      {/* Valid / Invalid banner */}
      {result.valid ? (<div className="bg-green-50 border border-green-300 rounded p-4" data-testid="valid-banner">
        ...
        </div>) : (<div className="bg-red-50 border border-red-300 rounded p-4" data-testid="invalid-banner">
          <div className="text-red-800 font-bold text-lg">
            &#10007; Verification failed
          </div>
          <ul className="text-red-700 text-sm mt-1 list-disc ml-4">
            {result.errors.map((err, i) => (<li key={i} data-testid={`error-${err}`}>
                {err}
              </li>))}
          </ul>
        </div>)}
      ...
      {witnessedRecords > 0 && (<div className="bg-indigo-50 border border-indigo-200 rounded p-4" data-testid="witnessed-section">
      ...
      {configDriftCount > 0 && (<div className="bg-amber-50 border border-amber-200 rounded p-4" data-testid="config-drift-notice">
          Configuration drift detected during this run. {configDriftCount}{" "}
          check(s) had changed manifest hashes.
        </div>)}

      {/* Warnings */}
      {result.warnings.length > 0 && (<div className="text-sm text-amber-600" data-testid="warnings">
          {result.warnings.map((w, i) => (<div key={i}>{w}</div>))}
        </div>)}
    </div>);
}
//# sourceMappingURL=VerificationPanel.js.map