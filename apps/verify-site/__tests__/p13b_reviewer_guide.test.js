...
describe("P13-B: Reviewer Guide", () => {
    it("MUST PASS: guide explains all 5 proof levels with clear distinction", () => {
        ...
        for (const level of levels) {
            expect(screen.getByTestId(`proof-level-guide-${level}`)).toBeInTheDocument();
        }
    ...
    it("MUST PASS: reviewer key never appears in any example that sends to Primust API", () => {
        ...
        expect(screen.getByTestId("key-never-sent").textContent).toContain("never sent to the Primust API");
...
//# sourceMappingURL=p13b_reviewer_guide.test.js.map