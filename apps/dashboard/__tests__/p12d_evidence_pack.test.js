...
describe("P12-D: Evidence Pack Assembler UI", () => {
    it("MUST PASS: local download works (default mode)", () => {
        ...
        render(<EvidencePackAssembler artifactIds={["vpec_001", "vpec_002"]} onAssembleLocal={onLocal} onAssembleHosted={onHosted}/>);
        ...
        expect(screen.getByTestId("local-badge").textContent).toContain("raw content does not leave your environment");
    ...
    it("MUST PASS: hosted requires acknowledgment", () => {
        const onLocal = vi.fn();
        const onHosted = vi.fn();
        render(<EvidencePackAssembler artifactIds={["vpec_001"]} onAssembleLocal={onLocal} onAssembleHosted={onHosted}/>);
...
//# sourceMappingURL=p12d_evidence_pack.test.js.map