...
describe("P12-C: Coverage Report + Policy Management", () => {
    it("MUST PASS: both denominators displayed separately (not collapsed)", () => {
        render(<CoverageReport policyCoveragePct={97.3} recordsPass={142} recordsTotal={146} instrumentationSurfacePct={100} instrumentationSurfaceBasis="LangGraph adapter" coverageVerifiedPct={90} coveragePendingPct={5} coverageUngovernedPct={5} proofDistribution={makeDistribution()} gapSummary={makeGapSummary()}/>);
    ...
    it("MUST PASS: partial_unknown scope → 'lower bound' warning", () => {
        render(<CoverageDisplay policyCoveragePct={80} recordsPass={80} recordsTotal={100} instrumentationSurfacePct={null} instrumentationSurfaceBasis={null} coverageVerifiedPct={80} coveragePendingPct={10} coverageUngovernedPct={10}/>);
        const warning = screen.getByTestId("lower-bound-warning");
        expect(warning).toBeInTheDocument();
        expect(warning.textContent).toContain("Scope partially unknown — coverage is a lower bound");
    });
    it("MUST PASS: coverage_verified + coverage_pending + coverage_ungoverned = 100 display", () => {
        render(<CoverageReport policyCoveragePct={90} recordsPass={90} recordsTotal={100} instrumentationSurfacePct={100} instrumentationSurfaceBasis="adapter" coverageVerifiedPct={70} coveragePendingPct={20} coverageUngovernedPct={10} proofDistribution={makeDistribution()} gapSummary={makeGapSummary()}/>);
    ...
    it("MUST PASS: all 5 proof levels in distribution chart", () => {
        render(<CoverageReport policyCoveragePct={90} recordsPass={90} recordsTotal={100} instrumentationSurfacePct={100} instrumentationSurfaceBasis="adapter" coverageVerifiedPct={90} coveragePendingPct={5} coverageUngovernedPct={5} proofDistribution={makeDistribution()} gapSummary={makeGapSummary()}/>);
    ...
    it("MUST PASS: all 5 gap severities in gap summary", () => {
        ...
        for (const sev of GAP_SEVERITIES) {
            expect(screen.getByTestId(`gap-summary-${sev.toLowerCase()}`)).toBeInTheDocument();
        }
    });
});
//# sourceMappingURL=p12c_coverage_report.test.js.map