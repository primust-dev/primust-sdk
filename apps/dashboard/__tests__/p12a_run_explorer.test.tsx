...
import { ManifestHashesTable } from "../src/components/ManifestHashesTable";
import type {
  ProcessRun,
  VPECArtifact,
  GapEntry,
...
function makeGap(type: GapType, severity = "High" as const): GapEntry {
  return {
    gap_id: `gap_${type}`,
    gap_type: type,
    severity,
...
describe("P12-A: Dashboard Foundation + Run Explorer", () => {
  it("MUST PASS: all 5 proof levels render in run list and detail", () => {
    ...
    const runs = levels.map((level) =>
      makeRun({ run_id: `run_${level}`, proof_level: level }),
    );
