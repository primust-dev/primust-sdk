...
import {
  ...
  type TestResult,
} from "../src/components/WebhookSettings";
...
function makeFailure(overrides: Partial<DeliveryFailure> = {}): DeliveryFailure {
  return {
    id: "fail_001",
    delivery_id: "del_001",
    vpec_id: "vpec_abc123",
...
function noopTest(): Promise<TestResult> {
  return Promise.resolve({ delivery_id: "del_test", status: 200, latency_ms: 42 });
}

function renderSettings(overrides: Partial<WebhookSettingsProps> = {}) {
  const props: WebhookSettingsProps = {
    config: makeConfig(),
    failures: [],
...
describe("P12-A: Dashboard Webhook Settings", () => {
  ...
  it("MUST PASS: WebhookStatusBanner hidden when webhook is healthy", () => {
    ...
    const { container } = render(
      <WebhookStatusBanner config={healthyConfig} />,
    );
