...
export default function OnboardPage() {
    ...
    return (<div className="mx-auto max-w-2xl py-12 px-4">
      ...
      {state.kind === "loading" && (<div className="py-8 text-center text-sm text-gray-500" data-testid="onboard-loading">
          Setting up your sandbox...
        </div>)}

      {state.kind === "error" && (<div className="rounded-lg border border-red-300 bg-red-50 p-4 text-sm text-red-800" data-testid="onboard-error">
          <p className="font-semibold">Onboarding failed</p>
          <p>{state.message}</p>
        </div>)}

      {state.kind === "already_existed" && (<div className="rounded-lg border border-blue-300 bg-blue-50 p-4 text-sm text-blue-800" data-testid="onboard-existing">
          <p className="font-semibold">Sandbox key already issued</p>
          <p className="mt-1">
            Your sandbox key was issued previously. You can view your keys in{" "}
            <a href="/settings/api-keys" className="font-medium underline hover:text-blue-900">
              Settings &gt; API Keys
            </a>
            .
          </p>
      ...
      {state.kind === "new_key" && (<div className="space-y-6">
...
vpec = client.vpecs.create(workflow_id="my-first-workflow")`}
              ...
              <li>
                ...
                <p className="mt-1">
                  See the{" "}
                  <a href="https://docs.primust.com/quickstart" target="_blank" rel="noopener noreferrer" className="font-medium text-indigo-600 underline hover:text-indigo-800">
                    Quickstart Guide
                  </a>{" "}
          ...
          <div className="text-center">
            <a href="/settings/api-keys" className="text-sm text-gray-500 hover:text-gray-700 underline">
              View all API keys
            </a>
          </div>
        </div>)}
