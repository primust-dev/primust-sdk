...
export interface WebhookConfig {
  ...
  id?: string;
  endpoint_url?: string;
  auth_header?: string; // always "••••••••" from API
  enabled?: boolean;
...
export function WebhookSettings({
  ...
  const [threshold, setThreshold] = useState(
    config.coverage_threshold_floor ?? 0.80,
  );
  ...
  if (!config.configured && !editing) {
    return (
      <div data-testid="webhook-empty-state" className="p-6 border rounded-lg">
        ...
        <button
          ...
          data-testid="webhook-configure-btn"
        >
          Configure Webhook
        </button>
      </div>
  ...
  if (config.configured && !editing) {
    ...
    return (
      <div data-testid="webhook-configured" className="space-y-6">
            ...
            >
              ...
              <div className="flex gap-2">
                <button
                  ...
                  data-testid="webhook-confirm-delete-btn"
                >
                  Delete
                </button>
                <button
        ...
        {/* Dead letter panel */}
        {failures.length > 0 && (
          <div className="p-6 border rounded-lg" data-testid="webhook-failures-panel">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {failures.slice(0, 10).map((f) => (
                  <tr key={f.id} className="border-b" data-testid="webhook-failure-row">
                    ...
                    <td className="py-2">
                      <button
                        onClick={() => handleRetry(f.delivery_id)}
                        className="text-blue-600 text-xs hover:underline"
                        data-testid="webhook-retry-btn"
                      >
                        Retry
                      </button>
                    </td>
  ...
  return (
    <div data-testid="webhook-form" className="space-y-6">
      <form onSubmit={handleSaveAndTest} className="p-6 border rounded-lg space-y-4">
        ...
        <div>
          ...
          <input
            ...
            placeholder="https://http-inputs.splunk.com/services/collector"
        ...
        <div>
          ...
          <input
            ...
            placeholder="Authorization: Splunk <HEC_TOKEN>"
            className="w-full border rounded p-2 text-sm font-mono"
            required
            data-testid="webhook-auth-input"
          />
          <p className="text-xs text-gray-500 mt-1">
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              data-testid="webhook-threshold-input"
            />
        ...
        <div>
          <button
            type="button"
            onClick={() => setShowSiemRef(!showSiemRef)}
            className="text-sm text-blue-600 hover:underline"
            data-testid="webhook-siem-ref-toggle"
          >
            {showSiemRef ? "Hide" : "Show"} SIEM auth header reference
          </button>
          {showSiemRef && config.siem_examples && (
            <table
              className="w-full text-xs mt-2 border"
              data-testid="webhook-siem-ref-table"
            >
              <thead>
              ...
              <tbody>
                {config.siem_examples.map((ex) => (
                  <tr key={ex.siem} className="border-t" data-testid="siem-example-row">
        ...
        <div>
          ...
          {showPayload && (
            <div className="mt-2" data-testid="webhook-payload-preview">
        ...
        <div>
          <button
            ...
            className="text-sm text-blue-600 hover:underline"
            data-testid="webhook-events-toggle"
          >
        ...
        <div className="flex gap-2">
          <button
            type="submit"
            disabled={saving || !endpointUrl || !authHeader}
            className={`px-4 py-2 rounded text-sm font-medium ${
              saving || !endpointUrl || !authHeader
