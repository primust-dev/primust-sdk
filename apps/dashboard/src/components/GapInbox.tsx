 ...
 */
export function GapInbox({ gaps }: GapInboxProps) {
        ...
        <tbody>
          {sorted.map((gap) => (
            <tr
              ...
              data-testid={`gap-inbox-row-${gap.gap_id}`}
            >
              ...
              <td className="py-2 px-2">
                <span
                  ...
                  data-testid={`gap-severity-${gap.gap_id}`}
                >
                  {gap.severity}
                </span>
              </td>
              <td className="py-2 px-2 font-mono text-xs">
                {(gap.details as Record<string, string>).run_id ?? "—"}
              </td>
