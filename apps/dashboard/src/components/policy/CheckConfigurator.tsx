...
import type { CheckDefinition } from "../../types/policy";
import type { ProofLevel } from "../../types/vpec";
import { PROOF_LEVEL_COLORS, PROOF_LEVEL_LABELS } from "../../lib/constants";
import { apiFetch } from "../../lib/api";

interface CheckConfiguratorProps {
  onAddCustomClick: () => void;
  refreshKey?: number;
}

export function CheckConfigurator({ onAddCustomClick, refreshKey }: CheckConfiguratorProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      try {
        const data = await apiFetch<CheckDefinition[]>("/policy/checks");
        if (!cancelled) {
          setChecks(data);
          setError(null);
        }
  ...
  function startEdit(check: CheckDefinition) {
    setEditingId(check.check_id);
    setEditValue(check.default_threshold?.toString() ?? "");
  }

  function commitEdit(checkId: string) {
    const parsed = parseFloat(editValue);
    if (!isNaN(parsed)) {
      setChecks((prev) =>
        prev.map((c) =>
          c.check_id === checkId ? { ...c, default_threshold: parsed } : c,
        ),
      );
    }
    setEditingId(null);
  ...
  function handleKeyDown(e: React.KeyboardEvent, checkId: string) {
    if (e.key === "Enter") commitEdit(checkId);
  ...
  if (loading) {
    return (
      <div className="p-4 text-sm text-gray-500" data-testid="check-configurator-loading">
        Loading checks...
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="p-4 text-sm text-red-600" data-testid="check-configurator-error">
        {error}
      </div>
    );
  }

  function renderTable(items: CheckDefinition[], label: string) {
    return (
      <div className="mb-6">
        <div className="flex items-center justify-between mb-2">
          ...
          {label === "Custom Checks" && (
            <button
              ...
              data-testid="add-custom-check-btn"
            >
              + Add Custom Check
            </button>
          )}
              ...
              <tbody>
                {items.map((check) => (
                  ...
                  >
                    ...
                    <td className="py-2 px-2">
                      <span
                        className={`inline-block rounded px-2 py-0.5 text-xs font-medium ${PROOF_LEVEL_COLORS[check.proof_ceiling as ProofLevel]}`}
                      >
                        {PROOF_LEVEL_LABELS[check.proof_ceiling as ProofLevel] ?? check.proof_ceiling}
                      </span>
                    </td>
                    ...
                    <td className="py-2 px-2">
                      {check.default_threshold !== undefined ? (
                        editingId === check.check_id ? (
                          <input
                            type="number"
                            value={editValue}
                            onChange={(e) => setEditValue(e.target.value)}
                            onBlur={() => commitEdit(check.check_id)}
                            onKeyDown={(e) => handleKeyDown(e, check.check_id)}
                            className="w-20 rounded border border-blue-400 px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-blue-400"
                            autoFocus
                            data-testid={`threshold-edit-${check.check_id}`}
                          />
                        ) : (
                          <button
                            type="button"
                            ...
                            data-testid={`threshold-display-${check.check_id}`}
                          >
                    ...
                    <td className="py-2 px-2 text-center">
                      {check.zk_available ? (
                        <span className="inline-block rounded bg-green-100 px-2 py-0.5 text-xs text-green-700 font-medium">
                          ZK
                        </span>
                      ) : (
  ...
  return (
    <div data-testid="check-configurator">
