...
import type { ActivePolicy, PolicyStatus } from "../../types/policy";
...
function StatusPill({ status }: { status: PolicyStatus }) {
  return (
    <span
      className={`inline-block rounded-full px-3 py-0.5 text-xs font-semibold uppercase tracking-wide ${STATUS_STYLES[status]}`}
      data-testid="policy-status-pill"
    >
      {status}
    </span>
  );
...
interface ActivePolicySummaryProps {
  onActivate?: () => void;
}

export function ActivePolicySummary({ onActivate }: ActivePolicySummaryProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const data = await apiFetch<ActivePolicy>("/policy/active");
  ...
  async function handleActivate() {
    ...
    setActivating(true);
    try {
      const updated = await apiFetch<ActivePolicy>("/policy/activate", {
        method: "POST",
  ...
  if (loading) {
    return (
      <div className="rounded border p-4 text-sm text-gray-500" data-testid="policy-summary-loading">
        Loading policy...
      </div>
    );
  }
  ...
  if (!policy) {
    return (
      <div className="rounded border border-dashed p-4" data-testid="policy-summary-empty">
  ...
  return (
    <div className="rounded border p-4" data-testid="active-policy-summary">
      ...
      <div className="space-y-2 text-sm">
        ...
        {policy.simulation_completed_at && (
          <div>
      ...
      <div className="mt-4">
        <button
          ...
          data-testid="activate-policy-btn"
        >
          {activating ? "Activating..." : "Activate"}
        </button>
        {!canActivate && policy.status === "simulation" && (
