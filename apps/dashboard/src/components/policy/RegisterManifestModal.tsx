...
interface RegisterManifestModalProps {
  open: boolean;
  onClose: () => void;
  onRegistered: () => void;
}
...
function validateManifest(data: unknown): { valid: boolean; errors: string[] } {
  ...
  if (typeof obj.name !== "string" || !obj.name) errors.push("Missing required field: name");
  if (typeof obj.check_name !== "string" || !obj.check_name) errors.push("Missing required field: check_name");
  if (typeof obj.version !== "string" || !obj.version) errors.push("Missing required field: version");
  if (typeof obj.proof_level_ceiling !== "string" || !obj.proof_level_ceiling) {
    ...
    if (!validLevels.includes(obj.proof_level_ceiling as string)) {
...
export function RegisterManifestModal({
  open,
  onClose,
  onRegistered,
}: RegisterManifestModalProps) {
  const [activeTab, setActiveTab] = useState<TabId>("upload");
  const [jsonText, setJsonText] = useState("");
  const [parsedManifest, setParsedManifest] = useState<ManifestPayload | null>(null);
  ...
  const processJson = useCallback(async (text: string) => {
    setJsonText(text);
    setSubmitError(null);
    try {
      ...
      if (result.valid) {
        setParsedManifest(data as ManifestPayload);
  ...
  return (
    <div
      ...
      data-testid="register-manifest-modal"
    >
      <div className="w-full max-w-xl rounded-lg bg-white shadow-xl">
        ...
        <div className="px-6 py-4">
          ...
          {activeTab === "paste" && (
            <textarea
              ...
              placeholder='{"name": "...", "check_name": "...", "version": "1.0.0", "proof_level_ceiling": "execution"}'
              className="w-full h-48 rounded border border-gray-300 px-3 py-2 text-sm font-mono focus:outline-none focus:ring-1 focus:ring-blue-400 focus:border-blue-400"
              data-testid="json-textarea"
            />
          )}

          {validationErrors.length > 0 && (
            <div className="mt-3 rounded bg-red-50 p-3" data-testid="validation-errors">
          ...
          {computedHash && parsedManifest && (
            <div className="mt-3 rounded bg-green-50 p-3" data-testid="manifest-preview">
          ...
          {submitError && (
            <div className="mt-3 rounded bg-red-50 p-3" data-testid="submit-error">
        ...
        <div className="flex justify-end gap-3 border-t px-6 py-4">
          ...
          <button
            ...
            disabled={!parsedManifest || submitting}
            className={`rounded px-4 py-2 text-sm font-medium transition-colors ${
              parsedManifest && !submitting
