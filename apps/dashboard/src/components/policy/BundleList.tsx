...
export function BundleList({ selectedBundleId, onSelectBundle }: BundleListProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const data = await apiFetch<PolicyBundle[]>("/policy/bundles");
        if (!cancelled) {
          setBundles(data);
          setLoading(false);
        }
  ...
  if (loading) {
    return (
      <div className="p-4 text-sm text-gray-500" data-testid="bundle-list-loading">
        Loading bundles...
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="p-4 text-sm text-red-600" data-testid="bundle-list-error">
        {error}
      </div>
    );
  }
  ...
  return (
    <div data-testid="bundle-list">
        ...
        {builtins.map((bundle) => (
          <li key={bundle.bundle_id}>
            <button
              ...
              data-testid={`bundle-item-${bundle.bundle_id}`}
            >
      ...
      <ul className="space-y-1 px-2">
        {custom.length === 0 && (
          <li className="px-3 py-2 text-xs text-gray-400">No custom bundles yet</li>
        )}
        {custom.map((bundle) => (
          <li key={bundle.bundle_id}>
            <button
      ...
      <div className="px-2 mt-4">
        <button
          type="button"
          className="w-full rounded border border-dashed border-gray-300 px-3 py-2 text-sm text-gray-600 hover:border-blue-400 hover:text-blue-600 transition-colors"
          data-testid="build-from-scratch-btn"
        >
          + Build from scratch
        </button>
      </div>
