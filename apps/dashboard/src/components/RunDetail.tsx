import type { VPECArtifact, CheckExecutionRecord } from "../types/vpec";
...
export function RunDetail({ vpec, records }: RunDetailProps) {
  return (
    <div className="space-y-6" data-testid="run-detail">
      ...
      <div className="flex items-center gap-3">
        ...
        {vpec.partial && (
          <span
            className="rounded bg-amber-200 px-2 py-0.5 text-xs font-medium"
      ...
      <section>
        ...
        <CoverageDisplay
          policyCoveragePct={vpec.policy_coverage_pct}
          recordsPass={vpec.records_pass}
          recordsTotal={vpec.records_total}
          instrumentationSurfacePct={vpec.instrumentation_surface_pct}
          instrumentationSurfaceBasis={vpec.instrumentation_surface_basis}
          coverageVerifiedPct={vpec.coverage_verified_pct}
          coveragePendingPct={vpec.coverage_pending_pct}
          coverageUngovernedPct={vpec.coverage_ungoverned_pct}
        />
      </section>

      {/* Surface summary */}
      {vpec.surface_summary.length > 0 && (
        <section>
      ...
      {vpec.process_context_hash && (
        <section>
          ...
          <span
            className="inline-block rounded bg-purple-100 px-2 py-0.5 text-xs font-mono"
      ...
      <section>
        ...
        {vpec.transparency_log?.rekor_log_id ? (
          <a
            href={`https://search.sigstore.dev/?logIndex=${vpec.transparency_log.rekor_log_id}`}
            className="text-blue-600 underline text-sm"
            data-testid="rekor-link"
          >
            {vpec.transparency_log.rekor_log_id}
          </a>
        ) : (
          <span
            className="inline-block rounded bg-yellow-200 px-2 py-0.5 text-xs"
            data-testid="rekor-pending-badge"
          >
            Pending
          </span>
        )}
              ...
              {records.map((rec) => (
                <tr key={rec.record_id}>
                  ...
                  <td className="py-1 px-2 text-xs">
                    {rec.check_open_tst && (
                      <span className="bg-blue-100 rounded px-1 mr-1">
                        open
                      </span>
                    )}
                    {rec.check_close_tst && (
                      <span className="bg-blue-100 rounded px-1">close</span>
                    )}
                  </td>
                  <td className="py-1 px-2">
                    {rec.reviewer_credential ? (
                      <span
                        className="inline-block rounded bg-indigo-100 px-1 text-xs"
                        data-testid="reviewer-credential-badge"
                      >
                        reviewed
                      </span>
                    ) : null}
...
function ZkProofBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-block rounded px-2 py-0.5 text-xs font-medium ${colors[status] ?? colors.none}`}
