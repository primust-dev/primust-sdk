...
export function RunList({ runs }) {
    return (<table className="w-full text-sm" data-testid="run-list">
      <thead>
      ...
      <tbody>
        {runs.map((run) => (<tr key={run.run_id} className="border-b">
            <td className="py-2 px-2 font-mono text-xs">
              {run.run_id}
              {run.partial && (<span className="ml-1 inline-block rounded bg-amber-200 px-1 text-xs" data-testid="partial-badge">
                  PARTIAL
                </span>)}
            </td>
            ...
            <td className="py-2 px-2">
              {run.proof_level ? (<ProofLevelBadge level={run.proof_level}/>) : ("—")}
            </td>
            <td className="py-2 px-2">
            ...
            <td className="py-2 px-2">
              {run.process_context_hash ? (<span className="inline-block rounded bg-purple-100 px-1.5 py-0.5 text-xs font-mono" data-testid="config-epoch-badge" title={run.process_context_hash}>
