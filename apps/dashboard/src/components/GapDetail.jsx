 ...
 */
export function GapDetail({ gap }) {
    const details = gap.details;
    return (<div className="space-y-4" data-testid="gap-detail">
      <div className="flex items-center gap-3">
        <h2 className="text-lg font-bold">{gap.gap_id}</h2>
        <span className={`inline-block rounded px-2 py-0.5 text-xs font-medium ${GAP_SEVERITY_COLORS[gap.severity]}`}>
          {gap.severity}
        </span>
      </div>
      ...
      {gap.gap_type === "policy_config_drift" ? (<div data-testid="config-drift-diff">
          ...
          <div className="text-sm space-y-1 bg-gray-50 p-2 rounded">
            <div>
              ...
              <code className="font-mono text-xs">
                {details.prior_hash ?? "unknown"}
              </code>
            </div>
            <div>
              <span className="text-green-600">+ Current: </span>
              <code className="font-mono text-xs">
                {details.current_hash ?? "unknown"}
              </code>
            </div>
          </div>
        </div>) : null}
      ...
      {(gap.gap_type === "reviewer_credential_invalid" ||
            ...
            details.reviewer_credential ? (<div data-testid="reviewer-credential-block">
...
//# sourceMappingURL=GapDetail.js.map