...
## v7.0 · March 15, 2026 · Supersedes TECH_SPEC v6.0 and all prior versions
## Canonical sources: DECISIONS_v12, ARCHITECTURE_v1, MASTER_v8

> New in v7: Cross-org verification checks (upstream_vpec_verify + 3 domain-neutral). Domain-neutral check examples. Audit Report API + CLI (§12). Four verification paths including relying party Path 0. AIUC-1 schema additions (§13). Unstructured check archetype spec (§14). 30 gap types. Gap taxonomy updated. Package inventory updated to shipped state March 15, 2026.
...
| policy-engine | TS | Policy snapshot binding, manifest validation, proof ceiling calculation, gap detection, VPEC issuance | Built. GREEN — tsc errors fixed. |
...
| evidence-pack | TS | Merkle pack assembly, verification instructions | Built. GREEN — tsc errors fixed. |
...
| db | SQL | 8 migrations | Built. All 8 migrations applied to both Neon regions. |
...
| primust-verify | Python | PyPI (Apache-2.0) | Live 1.0.0. GREEN — display string fixed. |
...
|---|---|---|---|---|
| primust-cedar | Java | AWS Cedar | Mathematical | Published 1.0.0. GREEN. |
| primust-drools | Java | Red Hat Drools | Mathematical | Published 1.0.0. GREEN. |
| primust-odm | Java | IBM ODM | Mathematical | Published 1.0.0. GREEN. |
| primust-opa | Go | OPA v1.4.2 | Mathematical (local) / Attestation (remote) | Tagged. GREEN — go.mod fixed. |

### 2.6 Regulated Connectors

| Package | Contents | Status |
|---|---|---|
| primust-connectors | 13 connectors across AML, credit, fraud, clinical, insurance | Guidewire: BUILT (38 tests). All others: deferred. |

### 2.7 Schemas (v4.0.0)

11 JSON schemas. Migration 003 covers most AIUC-1 fields. Migration 009 adds prompt fields to policy_snapshots.
...
| Proof level map values | primust-otel (py + js) | 15 | FIXED — test fixtures updated to canonical values |
| Verifier display string | primust-verify | 2 | FIXED — display label updated to 'Verifiable Inference' |
...
AMDT-16 resolved. primust-hook binary built (Go, 31 tests). See DECISION_hook_binary_amdt16.md.
...
## 15. GAP TAXONOMY — 30 TYPES (CANONICAL)
...
|---|---|---|
| upstream_vpec_invalid_signature | Critical | Ed25519 verification fails (exit 1) |
| upstream_vpec_sandbox | High | Upstream environment: "sandbox" (exit 2) |
| upstream_vpec_key_revoked | High | Signing key revoked (exit 3) |
...
*PRIMUST_TECH_SPEC_v7.0 · March 15, 2026 · Primust, Inc.*
*Supersedes TECH_SPEC v6.0 and all prior versions.*
*Canonical sources: DECISIONS_v12, ARCHITECTURE_v1, MASTER_v8*
