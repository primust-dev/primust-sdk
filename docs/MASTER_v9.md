...
### v9.0 · March 16, 2026 · Supersedes MASTER v8 and all prior versions

> **CRITICAL: THIS DOCUMENT WINS ALL CONFLICTS alongside DECISIONS_v12.**
> GEP · VPEC · Company: Primust · TrustScope: TABLED
> New in v9: VPEC field renames (proof_level_floor, provable_surface, gaps). Gap taxonomy 47 types. All 7 Python connectors confirmed built (321 tests). Guidewire clarified: Python REST / Attestation ceiling, Java spec only. visibility: "opaque" restored. pk_sb_ canonical. system_unavailable gap type. Connector license Apache-2.0.
...
|---|---|---|---|---|
| Mathematical | mathematical | Noir, UltraHonk, Modal CPU | None — math | Regex, allowlist, threshold, OPA (local) / Cedar eval() / Drools eval() + Map facts / ODM eval() via IlrSessionFactory, Ed25519 verify |
...
|---|---|---|
| PyPI | primust, primust-verify, primust-checks, primust-langgraph, primust-openai-agents, primust-google-adk, primust-otel, primust-connectors | Live 1.0.0 |
| npm | @primust/sdk, @primust/otel, @primust/artifact-core | Live 1.0.0 |
| Maven Central | primust-cedar 1.0.1, primust-drools 1.0.1, primust-odm 1.1.0 (Mathematical — real ODM runtime verified), sdk-java 1.0.0 | Live |
| Go | primust-opa v1.0.1, rules-core-go | Tagged. pkg.go.dev syncing. |
| app.primust.com | Dashboard + Policy Center + onboarding | Live |
| GitHub | primust-dev/primust | Public |

**Infrastructure — LIVE (March 16, 2026):**
- API: primust-api.fly.dev — status: ok, db: ok, kms: {us: ok, eu: ok}
- Migrations 001–011: Applied to Neon US + EU
- GCP KMS Ed25519: Deployed. KMS health check bug fixed (double cryptoKeyVersions suffix)
- app.primust.com: Live
- verify.primust.com: Live
...
- primust-hook (P29-A — AMDT-16 open questions)
- primust-verify-java (first Java relying-party design partner)
- Migration 012: Schema reconciliation (VPEC field renames, pk_sb_, gap taxonomy 47 types) — write before production data
- Domain packs (first FSI/Insurance/CI-CD design partner)

**primust-connectors (Apache-2.0) — 7 Python REST connectors BUILT (321 tests):**
ComplyAdvantage (48), NICE Actimize (51), FICO Blaze + IBM ODM (41), FICO Falcon (45), Pega CDH (46), Wolters Kluwer UpToDate (46), Guidewire ClaimCenter (38). All Attestation ceiling (REST wrappers). Java/C# in-process specs for Mathematical ceiling require vendor SDK licenses. Guidewire Java requires Guidewire Studio.
...
**Immediate:**
- Migration 009: Write AIUC-1 schema additions (§28 of DECISIONS_v12) before production data exists
- P15-A: primust discover CLI (gates archetype inference, check candidates)

**Next build:**
- P15-B: autoinstrument() runtime discovery (depends P15-A)
- primust verify-report CLI (part of audit report surface)
- P29-A: primust-hook (after AMDT-16 resolved)
...
- `visibility: "opaque"` is the default on all records. Connector records enforce opaque — not configurable by caller. Regulated data must never be transparent.
...
## 17. GAP TAXONOMY — 47 TYPES (CANONICAL)

Core (22) + system_unavailable (1) + archetype_unmapped (1) + cross-org (7) + connector-specific (16) = 47 total.

See DECISIONS_v13 §22 for full list with severities and all 47 types. check_candidate findings are NOT gap types — they are scan advisories.

---

*PRIMUST MASTER v9.0 · March 16, 2026 · Supersedes MASTER v8 and all prior versions.*
