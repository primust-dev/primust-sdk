 ...
 */
export function buildCoverageCheckWitness(
  runId: string,
  store: SqliteStore,
  thresholdPct: number,
): CoverageCheckInputs {
  ...
  if (records.length > MAX_ACTIONS) {
    throw new Error(
      `Record count ${records.length} exceeds MAX_ACTIONS ${MAX_ACTIONS}`,
    );
  }
  ...
  const hasRecord: number[] = [];

  for (const record of records) {
    actionHashes.push(record.commitment_hash as string);
    hasRecord.push(1);
  }
  ...
  if (coveredCount * 100 < thresholdPct * actionCount) {
    throw new Error(
      `Coverage ${coveredCount}/${actionCount} (${Math.round((coveredCount * 100) / actionCount)}%) ` +
