...
// ── Helpers ──
function makeProcessRun(overrides) {
    return {
        run_id: 'run_001',
        workflow_id: 'wf_001',
        org_id: 'org_001',
...
function makePriorVpec(processContextHash) {
    return {
        vpec_id: 'vpec_prior',
        ...
        proof_distribution: {
            ...
            weakest_link_explanation: 'No records',
        },
        state: 'final',
        coverage: {
            records_total: 0,
            ...
            instrumentation_surface_basis: 'No observation surface registered',
        },
        gaps: [],
        manifest_hashes: {},
        ...
        issuer: {
            ...
            kid: 'kid_001',
            algorithm: 'Ed25519',
            public_key_url: 'https://primust.com/.well-known/primust-pubkeys/kid_001.pem',
            org_region: 'us',
        },
        signature: {
            signer_id: 'signer_001',
...
describe('config_epoch_continuity witness builder', () => {
    ...
    it('MUST PASS: proves when hashes differ AND transition_gap_commitment is non-zero', () => {
        ...
        const witness = buildConfigEpochWitness(run, prior, CONFIG_PARAMS, BLINDING, gapCommitment);
    ...
    it('MUST PASS: throws when hashes differ AND transition_gap_commitment is zero', () => {
        const run = makeProcessRun();
        const prior = makePriorVpec('poseidon2:' + 'ff'.repeat(32)); // different hash
        expect(() => buildConfigEpochWitness(run, prior, CONFIG_PARAMS, BLINDING, 0n)).toThrow('no transition_gap_commitment');
    });
    ...
    it('MUST PASS: transition_commitment_hash binds gap commitment (soundness)', () => {
        ...
        const witness2 = buildConfigEpochWitness(run, prior, CONFIG_PARAMS, 43n, gapCommitment);
        expect(witness2.transition_commitment_hash).not.toBe(witness.transition_commitment_hash);
    });
...
//# sourceMappingURL=config_epoch_continuity.test.js.map