...
describe('skip_condition_proof witness builder', () => {
    ...
    it('MUST PASS: throws if condition_values are all zero', () => {
        ...
        expect(() => buildSkipConditionWitness(record, snapshot, allZeros, 99n)).toThrow('vacuous skip');
    });
    it('MUST PASS: neither witness builder contains TrustScope field names', () => {
        ...
        const files = fs.readdirSync(witnessesDir).filter((f) => f.endsWith('.ts') && !f.endsWith('.test.ts'));
...
//# sourceMappingURL=skip_condition_proof.test.js.map