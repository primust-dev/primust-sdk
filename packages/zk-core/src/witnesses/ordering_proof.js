 ...
 */
export function buildOrderingProofWitness(runId, store) {
    const records = store.getCheckRecords(runId);
    if (records.length > MAX_SEQUENCE) {
        throw new Error(`Record count ${records.length} exceeds MAX_SEQUENCE ${MAX_SEQUENCE}`);
    }
    if (records.length === 0) {
    ...
    for (let i = 0; i < sequenceValues.length - 1; i++) {
        if (sequenceValues[i] >= sequenceValues[i + 1]) {
            throw new Error(`ordering_proof: records not monotonically ordered at index ${i}`);
        }
    }
    ...
    const firstTs = BigInt(new Date(records[0].recorded_at).getTime());
    let chainHash = 'poseidon2:' + firstTs.toString(16).padStart(64, '0');
    if (count > 1) {
        for (let i = 1; i < count; i++) {
            const nextTs = BigInt(new Date(records[i].recorded_at).getTime());
...
//# sourceMappingURL=ordering_proof.js.map