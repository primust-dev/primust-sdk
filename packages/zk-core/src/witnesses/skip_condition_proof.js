 ...
 */
export function buildSkipConditionWitness(record, snapshot, conditionValues, blindingFactor) {
    // Validate: at least one condition value must be non-zero
    const hasNonZero = conditionValues.some((v) => v !== 0n);
    if (!hasNonZero) {
        throw new Error('skip_condition_proof: all condition_values are zero — vacuous skip cannot be proven');
    }
    // Pad condition_values to 16 elements
    const paddedValues = Array(16).fill(0n);
    ...
    const commitmentRoot = record.commitment_hash ?? 'poseidon2:' + '0'.repeat(64);
    ...
    return {
        ...
        policy_snapshot_hash: snapshot.policy_pack_id + ':' + snapshot.policy_pack_version,
    };
}
...
//# sourceMappingURL=skip_condition_proof.js.map