...
import { SyncQueue, ZK_IS_BLOCKING, QUEUE_MAX_RECORDS, QUEUE_RECORD_TTL_MS, CLOSE_MAX_RETRIES, } from './sync_queue.js';
import { SqliteStore } from '../store/sqlite_store.js';
// ── Helpers ──
function makeRecord(index, runId = 'run_001', overrides = {}) {
    return {
...
function createMockTarget(behavior = 'succeed') {
    const sent = [];
    return {
        sent,
        send: async (records) => {
...
function openDefaultRun(store, runId = 'run_001') {
    store.openRun({
        run_id: runId,
        workflow_id: 'wf_001',
        org_id: 'org_test',
...
describe('SyncQueue', () => {
    ...
    it('Mode 1: close retries with backoff and emits engine_error gap after max retries', async () => {
        ...
        const sleepCalls = [];
        const trackingSleep = async (ms) => { sleepCalls.push(ms); };
...
//# sourceMappingURL=sync_queue.test.js.map