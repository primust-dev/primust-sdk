...
export class SyncQueue {
  private buffer: QueuedRecord[] = [];
  private store: SqliteStore;
  private target: SyncTarget | null;
  private status: DegradedStatus;
  private callbacks: SyncQueueCallbacks;
  private pipelineTimers = new Map<string, ReturnType<typeof setTimeout>>();
  ...
  push(record: Omit<CheckExecutionRecord, 'chain_hash'>): void {
    try {
      ...
      const fullRecord: CheckExecutionRecord = {
        ...record,
   ...
   */
  startPipelineTtl(runId: string, ttlSeconds: number = PIPELINE_TTL_SECONDS): void {
   ...
   */
  private autoClose(runId: string): void {
    try {
      this.store.closeRun(runId, 'auto_closed');
      this.pipelineTimers.delete(runId);
    } catch (err) {
