...
function computeChainHash(prefix: string, content: string): string {
...
export class SqliteStore {
  ...
  openRun(run: {
    ...
    try {
      ...
      `).run(
        run.run_id, run.workflow_id, run.org_id, run.surface_id,
        run.policy_snapshot_hash, run.process_context_hash,
        run.action_unit_count, now, run.ttl_seconds,
      );
      ...
      if (run.manifest_hashes) {
        ...
        `).get(run.workflow_id, run.run_id) as { run_id: string } | undefined;
        ...
        if (lastRun) {
          for (const [manifestId, currentHash] of Object.entries(run.manifest_hashes)) {
            ...
            `).get(lastRun.run_id, manifestId) as { manifest_hash: string } | undefined;
  ...
  closeRun(runId: string, state: 'closed' | 'partial' | 'cancelled' | 'auto_closed' = 'closed'): void {
    try {
      this.db.prepare(`
  ...
  getProcessRun(runId: string): Record<string, unknown> | undefined {
    return this.db.prepare('SELECT * FROM process_runs WHERE run_id = ?').get(runId) as Record<string, unknown> | undefined;
  }
  ...
  writePolicySnapshot(snapshot: {
    ...
    try {
      ...
      `).run(
        ...
        snapshot.policy_pack_id,
        snapshot.policy_pack_version,
        JSON.stringify(snapshot.effective_checks),
        snapshot.snapshotted_at,
        snapshot.policy_basis,
      );
    } catch (err) {
  ...
  getPolicySnapshot(snapshotId: string): Record<string, unknown> | undefined {
    ...
    if (row) {
      row.effective_checks = JSON.parse(row.effective_checks as string);
    }
    return row;
  }
   ...
   */
  appendCheckRecord(
    record: Omit<CheckExecutionRecord, 'chain_hash'>,
    ...
    try {
      ...
      `).get(record.run_id) as { chain_hash: string } | undefined;

      let chainHash: string;
      if (prevRecord) {
        chainHash = computeChainHash(
          CHAIN_GENESIS_PREFIX,
          prevRecord.chain_hash + canonicalContent,
        );
      } else {
        chainHash = computeChainHash(
          CHAIN_GENESIS_PREFIX,
          record.run_id + canonicalContent,
        );
      }
      ...
      `).run(
        ...
        record.commitment_hash, record.output_commitment,
        record.commitment_algorithm, record.commitment_type,
        record.check_result, record.proof_level_achieved,
        record.proof_pending ? 1 : 0, record.zkml_proof_pending ? 1 : 0,
        record.check_open_tst, record.check_close_tst,
        record.skip_rationale_hash,
        record.reviewer_credential ? JSON.stringify(record.reviewer_credential) : null,
        record.unverified_provenance ? 1 : 0, record.freshness_warning ? 1 : 0,
        chainHash, record.idempotency_key, record.recorded_at,
      );
  ...
  getCheckRecords(runId: string): Record<string, unknown>[] {
    ...
    return rows.map((row) => ({
      ...
      reviewer_credential: row.reviewer_credential
        ? JSON.parse(row.reviewer_credential as string)
        : null,
    }));
  }
   ...
   */
  verifyChain(runId: string): { valid: boolean; brokenAt: number } {
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      if (i === 0) {
        expectedHash = computeChainHash(
          CHAIN_GENESIS_PREFIX,
          runId + canonicalContent,
        );
      } else {
        const prevHash = records[i - 1].chain_hash as string;
        expectedHash = computeChainHash(
          CHAIN_GENESIS_PREFIX,
          prevHash + canonicalContent,
        );
      }
  ...
  insertGap(gap: Gap): void {
    try {
      this.db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `).run(
        gap.gap_id, gap.run_id, gap.gap_type, gap.severity,
        gap.state, JSON.stringify(gap.details), gap.detected_at, gap.resolved_at,
      );
    } catch (err) {
  ...
  private buildRecordContent(record: Omit<CheckExecutionRecord, 'chain_hash'>): Record<string, unknown> {
    return {
  ...
  private buildRecordContentFromRow(row: Record<string, unknown>): Record<string, unknown> {
    return {
