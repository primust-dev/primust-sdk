...
import { scanBannedFields, validateManifestStage, validateCheckExecutionRecord, validateWaiver, validateEvidencePack, } from './validate-schemas.js';
// ── Helpers ──
function makeCheckRecord(overrides = {}) {
    return {
        record_id: 'rec_001',
        run_id: 'run_001',
...
describe('validateCheckExecutionRecord', () => {
    ...
    it('rejects missing manifest_hash (invariant 3)', () => {
        const errors = validateCheckExecutionRecord(makeCheckRecord({ manifest_hash: '' }));
        expect(errors.some((e) => e.code === 'manifest_hash_missing')).toBe(true);
    });
    it('rejects witnessed without reviewer_credential (invariant 4)', () => {
        const errors = validateCheckExecutionRecord(makeCheckRecord({
    ...
    it('rejects output_commitment without poseidon2 prefix', () => {
        const errors = validateCheckExecutionRecord(makeCheckRecord({ output_commitment: 'sha256:' + 'a'.repeat(64) }));
...
describe('validateWaiver', () => {
    ...
    it('rejects expires_at beyond 90 days (invariant 7)', () => {
        const errors = validateWaiver(makeWaiver({
            approved_at: '2026-03-10T00:00:00Z',
...
describe('validateEvidencePack', () => {
    ...
    it('rejects when coverage does not sum to 100', () => {
        const errors = validateEvidencePack(makeEvidencePack({
            coverage_verified_pct: 80,
            coverage_pending_pct: 15,
