...
describe('lineage tokens (P7-C)', () => {
    it('MUST PASS: valid token → validateLineageToken returns valid: true', () => {
        const token = generateLineageToken('run_001', 'surf_001', makeContext(), HMAC_KEY);
    ...
    it('MUST PASS: expired token → valid: false, errors: ["token_expired"]', () => {
        const token = generateLineageToken('run_001', 'surf_001', makeContext(), HMAC_KEY, 3600);
        ...
        const reissued = generateLineageToken('run_001', 'surf_001', makeContext(), HMAC_KEY, -3600);
    ...
    it('MUST PASS: token format includes external_boundary_traversal gap context (delegation scenario)', () => {
        ...
        const token = generateLineageToken('run_001', 'surf_001', context, HMAC_KEY);
    ...
    it('MUST PASS: token uses HMAC-SHA256 (not Ed25519 signing key)', () => {
        ...
        const tokenHmac = generateLineageToken('run_001', 'surf_001', makeContext(), HMAC_KEY);
