 ...
 */
export type ManifestDomain = 'ai_agent' | 'cicd' | 'financial' | 'pharma' | 'generic';
export type ImplementationType = 'ml_model' | 'rule' | 'threshold' | 'approval_chain' | 'zkml_model' | 'custom';
 ...
 */
export type StageType = 'deterministic_rule' | 'ml_model' | 'zkml_model' | 'statistical_test' | 'custom_code' | 'witnessed' | 'policy_engine' | 'llm_api' | 'open_source_ml' | 'hardware_attested';
export type EvaluationScope = 'per_run' | 'per_action_unit' | 'per_surface' | 'per_window';
export type AggregationMethod = 'all_stages_must_pass' | 'worst_case' | 'threshold_vote' | 'sequential_gate';
export type CheckResult = 'pass' | 'fail' | 'error' | 'skipped' | 'degraded' | 'override' | 'not_applicable' | 'timed_out';
export type GapState = 'open' | 'investigating' | 'waived' | 'remediated' | 'resolved' | 'escalated';
export type RunState = 'open' | 'closed' | 'partial' | 'cancelled' | 'auto_closed';
export type CommitmentType = 'input_commitment' | 'metadata_commitment' | 'foreign_event_commitment';
