  ...
  switch (typeof value) {
    ...
    case 'number':
      if (!Number.isFinite(value)) {
        throw new TypeError(
          `canonical: cannot serialize ${value} (NaN/Infinity are not valid JSON)`,
        );
      }
      return JSON.stringify(value);
    ...
    case 'object':
      ...
      if (value instanceof Date) {
        throw new TypeError(
          'canonical: Date objects must be converted to ISO 8601 strings before serialization',
        );
      }
      if (value instanceof Uint8Array || (typeof Buffer !== 'undefined' && Buffer.isBuffer(value))) {
        throw new TypeError(
          'canonical: byte arrays must be base64url-encoded before serialization',
        );
      }
