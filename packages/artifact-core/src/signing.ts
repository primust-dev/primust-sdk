...
): { document: Record<string, unknown>; signatureEnvelope: SignatureEnvelope } {
 ...
 */
export function rotateKey(existingRecord: SignerRecord): {
  ...
  if (existingRecord.status !== 'active') {
    throw new Error(
      `Cannot rotate ${existingRecord.status} key (kid: ${existingRecord.kid}). Only active keys can be rotated.`,
    );
  }

  // Generate new key pair under the same signer_id
  const { signerRecord: newRecord, privateKey: newPrivateKey } = generateKeyPair(
    existingRecord.signer_id,
    existingRecord.org_id,
    existingRecord.signer_type,
  );

  const now = new Date().toISOString();

  // Transition old record to rotated
  const updatedRecord: SignerRecord = {
    ...existingRecord,
    status: 'rotated',
