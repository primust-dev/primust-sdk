...
import { generateKeyPair, sign, } from '@primust/artifact-core';
 ...
 */
function buildArtifactBody(kid, publicKeyB64Url, overrides = {}) {
    const base = {
 ...
 */
function createSignedArtifact(overrides = {}) {
    const { signerRecord, privateKey } = generateKeyPair('signer_test', 'org_test', 'artifact_signer');
 ...
 */
function writeTrustRoot(publicKeyB64Url) {
...
describe('verify()', () => {
    ...
    it('MUST PASS: proof_level != weakest_link → valid: false', async () => {
        ...
        expect(result.errors.some((e) => e.includes('PROOF_LEVEL_MISMATCH'))).toBe(true);
    });
...
// ── CLI MUST PASS tests ──
describe('CLI (main)', () => {
    function writeArtifactFile(artifact) {
