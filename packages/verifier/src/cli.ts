...
function formatProofLevel(level: string): string {
...
function formatGapsSummary(gaps: Array<{ severity: string }>): string {
...
function formatTimestamp(result: VerificationResult): string {
...
function printHumanResult(result: VerificationResult): void {
  const dist = result.proof_distribution as Record<string, unknown>;
  const cov = result.coverage as Record<string, unknown>;
...
export async function main(args?: string[]): Promise<number> {
  ...
  try {
    ...
    const msg = `Error: verification failed: ${(err as Error).message}`;
    if (jsonOutput) {
      console.error(msg);
    } else {
      console.error(msg);
