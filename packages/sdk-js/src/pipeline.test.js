...
import { commit, commitOutput, ZK_IS_BLOCKING, } from '@primust/artifact-core';
...
function createPipeline(mockFetch, overrides = {}) {
    return new Pipeline({
...
describe('TypeScript SDK (P10-B)', () => {
    ...
    it('MUST PASS: check_open_tst fetched at openCheck(), check_close_tst at record()', async () => {
        ...
        expect(new Date(body.check_open_tst).getTime()).not.toBeNaN();
        expect(new Date(body.check_close_tst).getTime()).not.toBeNaN();
    });
    it('MUST PASS: skip_rationale_hash, display_hash, rationale_hash committed locally', async () => {
        ...
        const rawBody = recordReq[0].rawBody;
        // Hashes present
        const cred = body.reviewer_credential;
    ...
    it('MUST PASS: sub-threshold review duration throws check_timing_suspect', async () => {
        ...
        await expect(p.record(session, 'data', 'pass', { reviewerSignature: 'sig' })).rejects.toThrow('check_timing_suspect');
    });
