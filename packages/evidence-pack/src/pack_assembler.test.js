...
import { generateKeyPair, sign, buildCommitmentRoot, } from '@primust/artifact-core';
...
describe('Evidence Pack assembly (P8-A)', () => {
    it('MUST PASS: one signed artifact → one Evidence Pack', async () => {
        ...
        const pack = await assemblePack([artifact], '2026-03-01', '2026-03-31', 'org_001', signerRecord, privateKey, { coverage_verified_pct: 100, coverage_pending_pct: 0, coverage_ungoverned_pct: 0 });
    ...
    it('MUST PASS: Merkle root changes if any artifact is tampered', async () => {
        ...
        const pack1 = await assemblePack([artifact1], '2026-03-01', '2026-03-31', 'org_001', signerRecord, privateKey, { coverage_verified_pct: 100, coverage_pending_pct: 0, coverage_ungoverned_pct: 0 });
        const pack2 = await assemblePack([artifact2], '2026-03-01', '2026-03-31', 'org_001', signerRecord, privateKey, { coverage_verified_pct: 100, coverage_pending_pct: 0, coverage_ungoverned_pct: 0 });
    ...
    it('MUST PASS: coverage_verified + coverage_pending + coverage_ungoverned = 100 (assertion fires)', async () => {
        const artifact = makeArtifact();
        await expect(assemblePack([artifact], '2026-03-01', '2026-03-31', 'org_001', signerRecord, privateKey, { coverage_verified_pct: 50, coverage_pending_pct: 30, coverage_ungoverned_pct: 10 })).rejects.toThrow('must sum to 100');
    });
    it('MUST PASS: pack rejected if any artifact fails P2-A verification', async () => {
        const artifact = makeArtifact();
        // Tamper with artifact to break signature
        artifact.org_id = 'tampered_org';
        await expect(assemblePack([artifact], '2026-03-01', '2026-03-31', 'org_001', signerRecord, privateKey, { coverage_verified_pct: 100, coverage_pending_pct: 0, coverage_ungoverned_pct: 0 })).rejects.toThrow('failed verification');
    });
    ...
    it('MUST PASS: manifest_hashes aggregated from all artifacts (map not array)', async () => {
        ...
        const pack = await assemblePack([artifact1, artifact2], '2026-03-01', '2026-03-31', 'org_001', signerRecord, privateKey, { coverage_verified_pct: 100, coverage_pending_pct: 0, coverage_ungoverned_pct: 0 });
...
describe('Evidence Pack CLI and verification instructions (P8-B)', () => {
    it('MUST PASS: --dry-run prints "Raw content: NONE"', () => {
        const output = dryRunOutput(['vpec_abc123', 'vpec_def456', 'vpec_ghi789'], '2026-03-01', '2026-03-31', 98.7, 0, 1.3, 'in_process_adapter (LangGraph) — full_workflow', 147);
    ...
    it('MUST PASS: --dry-run makes zero API calls', () => {
        ...
        const output = dryRunOutput(['vpec_001'], '2026-03-01', '2026-03-31', 100, 0, 0, 'in_process_adapter — full_workflow', 50);
...
//# sourceMappingURL=pack_assembler.test.js.map