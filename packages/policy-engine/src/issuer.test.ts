...
import { generateKeyPair, validateArtifact, verify } from '@primust/artifact-core';
import type { VPECArtifact, ProofLevel, SignerRecord } from '@primust/artifact-core';
import { SqliteStore } from '@primust/runtime-core';
import type { CheckExecutionRecord, ReviewerCredential } from '@primust/runtime-core';
...
describe('VPEC issuance (P7-A)', () => {
  ...
  it('MUST PASS: artifact verifies via validateArtifact()', () => {
    ...
    const vpec = closeRun('run_001', store, signerRecord, privateKey);
    const result = validateArtifact(vpec as unknown as Record<string, unknown>);
