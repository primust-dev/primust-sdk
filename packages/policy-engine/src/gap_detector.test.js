...
describe('gap_detector', () => {
    ...
    it('MUST PASS: check_not_executed → High', () => {
        ...
        const notExecuted = gaps.find((g) => g.gap_type === 'check_not_executed' && g.details.manifest_id === 'manifest_002');
    ...
    it('MUST PASS: check_timing_suspect only fires when both timestamps present', () => {
        ...
        store.appendCheckRecord(makeRecord({
    ...
    it('MUST PASS: all 5 proof levels referenced correctly in proof_level_achieved checks', () => {
        openTestRun();
        const proofLevels = ['mathematical', 'verifiable_inference', 'execution', 'witnessed', 'attestation'];
        ...
        const witnessedGaps = gaps.filter((g) => g.gap_type === 'witnessed_display_missing' || g.gap_type === 'witnessed_rationale_missing');
        expect(witnessedGaps).toHaveLength(0);
    });
});
//# sourceMappingURL=gap_detector.test.js.map