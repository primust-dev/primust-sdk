...
// ── Helpers ──
function makeGap(runId, gapType, details = {}) {
 ...
 */
export function detectGaps(runId, store, policySnapshot, manifests, complianceRequirements) {
    const gaps = [];
    ...
    if (policySnapshot) {
        ...
        for (const check of policySnapshot.effective_checks) {
            if (check.required && !recordedManifestIds.has(check.manifest_id)) {
                gaps.push(makeGap(runId, 'check_not_executed', {
    ...
    for (const record of records) {
        ...
        if (record.check_result === 'override') {
            gaps.push(makeGap(runId, 'enforcement_override', {
        ...
        if (record.check_result === 'error') {
            gaps.push(makeGap(runId, 'engine_error', {
                record_id: record.record_id,
                manifest_id: record.manifest_id,
            }));
        }
        ...
        if (record.check_result === 'degraded') {
            gaps.push(makeGap(runId, 'check_degraded', {
                record_id: record.record_id,
                manifest_id: record.manifest_id,
            }));
        }
        ...
        if (record.check_result === 'not_applicable' &&
            ...
            gaps.push(makeGap(runId, 'skip_rationale_missing', {
                record_id: record.record_id,
                manifest_id: record.manifest_id,
            }));
        }
        ...
        if (record.check_open_tst && record.check_close_tst) {
            ...
            if (implType === 'ml_model' || implType === 'zkml_model') {
                ...
                if (!isNaN(openTime) && !isNaN(closeTime) && closeTime - openTime < 100) {
                    gaps.push(makeGap(runId, 'check_timing_suspect', {
                        record_id: record.record_id,
                        manifest_id: record.manifest_id,
        ...
        if (record.proof_level_achieved === 'witnessed') {
            const cred = record.reviewer_credential;
            if (cred) {
                // witnessed_display_missing
                if (!cred.display_hash) {
                    gaps.push(makeGap(runId, 'witnessed_display_missing', {
                        record_id: record.record_id,
                    }));
                }
                // witnessed_rationale_missing
                if (!cred.rationale_hash) {
                    gaps.push(makeGap(runId, 'witnessed_rationale_missing', {
                        record_id: record.record_id,
                    }));
                }
                // reviewer_credential_invalid — stub: check signature field exists
                if (!cred.reviewer_signature) {
                    gaps.push(makeGap(runId, 'reviewer_credential_invalid', {
                        record_id: record.record_id,
    ...
    const commitmentMap = new Map(); // "commitment_hash|manifest_id" → check_result
    for (const record of records) {
        ...
        if (prev !== undefined && prev !== record.check_result) {
            gaps.push(makeGap(runId, 'deterministic_consistency_violation', {
                manifest_id: record.manifest_id,
    ...
    if (complianceRequirements) {
        for (const record of records) {
            ...
            if (complianceRequirements.require_explanation_commitment) {
                ...
                const resultMatches = req.on_check_result.includes(record.check_result);
                const typeMatches = req.on_check_types.length === 0 ||
                    req.on_check_types.includes(manifests?.get(record.manifest_id)?.implementation_type ?? '');
                if (resultMatches &&
                    typeMatches &&
                    !record.explanation_commitment) {
                    gaps.push(makeGap(runId, 'explanation_missing', {
                        record_id: record.record_id,
                        manifest_id: record.manifest_id,
            ...
            if (complianceRequirements.require_bias_audit) {
                ...
                if (req.on_check_types.includes(implType) && !record.bias_audit) {
                    gaps.push(makeGap(runId, 'bias_audit_missing', {
                        record_id: record.record_id,
                        manifest_id: record.manifest_id,
                        check_type: implType,
                    }));
    ...
    const existingGaps = store.getGaps(runId);
...
export function getGapSeverity(gapType) {
...
//# sourceMappingURL=gap_detector.js.map